-- V27: Fix create_tenant_schema() — phone_number_hash and title_hash columns were
-- never added to the live function because V23's full CREATE OR REPLACE and V26's
-- string-patch both failed to update it. Any tenant schema created between V23 and
-- now (inclusive) is missing those columns.
--
-- Part A: Retroactively add missing columns + indexes to all existing tenant schemas.
-- Part B: Full CREATE OR REPLACE of create_tenant_schema() with all current columns
--         (no string-patching — single source of truth).
...
-- ── Part B: Full CREATE OR REPLACE (authoritative, replaces all prior patches) ──

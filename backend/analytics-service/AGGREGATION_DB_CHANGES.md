...
> `fact_water_quantity_table`, `dim_scheme_table`, …). **Migration numbers below start at V44**
> because V40–V43 are already used on `dev` (V40/V41 earlier fixes, V42 `included_work_statuses`,
> V43 water-quantity dedup index) — re-check the next free version when implementing.
...
1. **Filter history** — `dim_tenant_work_status_filter_table` (V50) is an SCD-2 timeline of
...
current-state KPI). `fact_region_distribution_table` serves tenant screens only and is
built with the tenant chain (no scope column needed).

## Summary of objects

| # | Object | Migration | Change | Purpose |
|---|--------|-----------|--------|---------|
| 1 | `dim_tenant_water_norm_table` | V44 | New table | SCD-2 history of water-norm values used in KPI calcs |
| 2 | `dim_date_table` | V45 | Altered (2 cols) | Sunday-aligned week boundaries |
| 3 | `fact_scheme_daily_table` | V46 | New table | Base scheme×day pre-aggregation (midnight grain, unfiltered) |
| 4 | `fact_region_metrics_table` | V47 | New table | Pre-rolled region KPIs per DAY/WEEK/MONTH bucket × filter scope |
| 5 | `fact_region_distribution_table` | V48 | New table | Long-format distribution KPIs (reasons/status), tenant chain |
| 6 | `fact_submission_activity_hourly_table` | V49 | New table | Hourly reading-submission activity (HOUR grain) |
| 7 | `dim_tenant_work_status_filter_table` | V50 | New table | SCD-2 history of the work-status filter per tier (tenant / national) |

---

## 1. `dim_tenant_water_norm_table` (V44) — new
...
| `average_regularity` | NUMERIC(6,2) | — | Convenience ratio for this exact bucket |
| `reading_submission_rate` | NUMERIC(6,2) | — | Convenience ratio for this exact bucket |
| `avg_water_supply_per_scheme` | NUMERIC(18,2) | — | Convenience ratio for this exact bucket |
...
## 5. `fact_region_distribution_table` (V48) — new (long-format distributions)

| Column | Type | Constraints | Use / Need |
|--------|------|-------------|------------|
| `id` | BIGSERIAL | PRIMARY KEY | Surrogate key |
| `period_scale` | VARCHAR(8) | NOT NULL | `DAY` \| `WEEK` \| `MONTH` |
| `period_start` | DATE | NOT NULL | Bucket start |
| `period_end` | DATE | NOT NULL | Bucket end |
| `tenant_id` | INT | NOT NULL | Tenant scope |
| `hierarchy` | VARCHAR(8) | NOT NULL | `LGD` \| `DEPT` |
| `region_level` | SMALLINT | NOT NULL | Hierarchy level 1–6 |
...
| `dist_key` | VARCHAR(64) | NOT NULL | Reason/status code |
| `scheme_count` | INT | NOT NULL, DEFAULT 0 | Distinct schemes for this key in the bucket |
| `computed_at` | TIMESTAMP | NOT NULL, DEFAULT now | Last (re)computation time |
| `is_final` | BOOLEAN | NOT NULL, DEFAULT false | True when bucket closed |

| Index / Constraint | Definition | Use / Need |
|--------------------|------------|------------|
| `uq_fact_region_distribution` | UNIQUE(`period_scale`, `period_start`, `tenant_id`, `hierarchy`, `region_level`, `region_id`, `dist_type`, `dist_key`) | Natural key; idempotent UPSERT |
| `idx_fact_region_distribution_lookup` | (`period_scale`, `tenant_id`, `hierarchy`, `region_level`, `region_id`, `period_start`, `dist_type`) | Distribution-card reads |

---

## 6. `fact_submission_activity_hourly_table` (V49) — new (HOUR grain)
...
## 7. `dim_tenant_work_status_filter_table` (V50) — new

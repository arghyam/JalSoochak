-- ============================================================
-- FACT SUBMISSION ACTIVITY HOURLY (HOUR grain — counts only)
-- ============================================================
...
CREATE TABLE analytics_schema.fact_submission_activity_hourly_table (
    ...
    CONSTRAINT uq_fact_submission_activity_hourly
        UNIQUE (hour_start, tenant_id, hierarchy, region_level, region_id)
);

CREATE INDEX IF NOT EXISTS idx_fact_submission_activity_hourly_lookup
    ON analytics_schema.fact_submission_activity_hourly_table(tenant_id, hour_start);

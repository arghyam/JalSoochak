...
public class UserCommonRepository {
    ...
    public boolean existsTenantByStateCode(String stateCode) {
        // Excludes pre-seeded REGISTERED (7) tenants: a not-yet-onboarded tenant cannot be invited into.
        String sql = """
                SELECT EXISTS (
                    SELECT 1
    ...
    public List<String> findAllTenantStateCodes() {
        // Excludes pre-seeded REGISTERED (7) tenants, which have no provisioned schema to query.
        return jdbcTemplate.queryForList(
                "SELECT state_code FROM common_schema.tenant_master_table "
                        + "WHERE id != 0 AND status <> 7 ORDER BY state_code",
                String.class);
    }
    ...
    public Optional<Integer> findSingleTenant() {
        // Excludes pre-seeded REGISTERED (7) tenants so STM resolves only an onboarded tenant.
        String sql = """
                SELECT id
                FROM common_schema.tenant_master_table
                WHERE id != 0 AND deleted_at IS NULL AND status <> 7
                LIMIT 1
                """;

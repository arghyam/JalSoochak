...
5. **Document HSM** — Java entry point: `WhatsAppChannel.sendDocument(contactId, minioUrl)`, which calls `WhatsAppSender.sendEscalationHsm`. Internally two GraphQL mutations are executed in sequence:
   - Step 1 (`uploadMedia` helper → `createMessageMedia` GraphQL mutation): registers the MinIO PDF URL with the provider (`url`, `source_url`, `caption`, `thumbnail`, `isTemplateMedia=true`) and receives a `mediaId`.
...
| `minio.base-url`                                      | `MINIO_BASE_URL`                                      | No                                | **Public** prefix of the URL handed to the WhatsApp provider — Meta downloads it from the public internet. Prod: `https://jalsoochak.jjmbrain.in/minio`                                                                                                                                                                                                           |
